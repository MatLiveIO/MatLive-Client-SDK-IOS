...
public class MatLiveRoomManager: ObservableObject {
    ...
    public var seatService: RoomSeatService?
    ...
    private var onInvitedToMic: ((Int) -> Void)?
    private var onSendGift: ((String) -> Void)?
    ...
    public func initialize(appKey:String,onInvitedToMic: ((Int) -> Void)? = nil, onSendGift: ((String) -> Void)? = nil) async {
        Utils.appKey = appKey
    ...
    public func close() async {
        ...
        LiveRoomEventReceiverManager.shared.inviteRequests = []
        room?.remove(delegate: self)
        await room?.disconnect()
    }


    private func initLocalAudioTrack() async{
        audioTrack = LocalAudioTrack.createTrack(options: AudioCaptureOptions(echoCancellation: true,noiseSuppression: true) )
    }
    ...
    private func createToken(roomId:String,userName:String,completion:@escaping(String,String)->Void)  async throws {
        let response =  await matLiveService.createToken(userName: userName, roomId: roomId)
    ...
    public func connect(
        ...
        do {
            try await createToken(roomId: roomId, userName: name) { [weak self] newId, token in
